...
# coding: utf-8
# Subdomain Scan v1
# By Emilyano Zapota - spyhackerz.org
...
╚══════╝╚═╝     ╚═╝╚═╝╚══════╝╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═╝      ╚═════╝    ╚═╝   ╚═╝  ╚═╝
                ...
                # Coded By Emilyano Zapota - Mectruy - Roselly - spyhackerz.org Version 1.0 / 2020 Sub Domain Scanner
                Ornek Kullanım python emilyano.py -d www.site.com
    """ % (R, W, Y))
...
def subdomain_sorting_key(hostname):
    """Alt alanlar için sıralama anahtarı

     Bu sıralama anahtarı, alt alan adlarını sağdaki üst düzey alan adından sipariş eder
     sola doğru okuduktan sonra '^' ve 'www' gruplarının en üstüne taşıyın. İçin
     sitesinde, aşağıdaki liste doğru şekilde sıralanır:
...
class enumratorBase(object):
    ...
    def print_banner(self):
        """ süslü bir banner istiyorlarsa alt sınıf bunu geçersiz kılabilir :)"""
        self.print_(G + "[-] Arama Simdi Basladi %s.." % (self.engine_name) + W)
        return
    ...
    def extract_domains(self, resp):
        """ alt sınıf bu işlevi geçersiz kılmalı """
        return

    # override
    def check_response_errors(self, resp):
        """ child sınıfı bu işlevi geçersiz kılmalı ve hata yoksa işlev True döndürmeli, aksi halde False
        """
        return True

    def should_sleep(self):
        """Bazı numaralandırıcılar, Google numaralandırıcı gibi bot algılamasını önlemek için uyku gerektiriyordu"""
        return
    ...
    def get_page(self, num):
        """ farklı sayfa sayacı kullanan alt sınıf bu işlevi geçersiz kılmalıdır """
        return num + 10
...
class Virustotal(enumratorBaseThreaded):
    ...
    def enumerate(self):
        while self.url != '':
            ...
            if 'error' in resp:
                self.print_(R + "[!] Hata: Hedef Site Prosedurlere Uygun Bir Site Degil Yada Tekrar Taramayı Dene" + W)
                break
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if not silent:
        print(B + "[-] Hedef Sitede Sub Domain Aranıyor %s" % parsed_domain.netloc + W)
    ...
    if subdomains:
        ...
        if not silent:
            print(Y + "[-] Bulunan Sub Domain Listesi: %s" % len(subdomains) + W)
